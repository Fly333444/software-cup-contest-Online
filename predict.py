...
class PredictConfig:
    def __init__(self, model_dir):
        ...
        self.arch = yml_conf.get('arch', 'GFL')
...
def get_test_images(infer_file):
    ...
    for line in lines:
        ...
        if not os.path.isabs(line):
            line = os.path.join(SCRIPT_DIR, line)
        images.append(line)
...
def load_predictor(model_dir):
    ...
    # Try TensorRT FP16 for max FPS; fall back gracefully if unavailable
    try:
        ...
        print('TensorRT FP16 engine enabled')
    except Exception:
        print('TensorRT not available, using native GPU inference')
...
def create_inputs(imgs, im_info, input_names):
    """Create inputs matching the exported model's expected inputs.

    Since Resize with keep_ratio=False produces identically-sized images,
    we skip the padding loop and stack directly for better performance.
    """
    inputs = {}
...
class Detector(object):
    ...
    def predict(self, imgs, im_info):
        ...
        # PicoDet with export_nms=True outputs:
        #   output_names[0] -> bboxes [N, 6] with [class_id, score, x1, y1, x2, y2]
        #   output_names[num_outs] -> bbox_num [1]
...
def predict_image(detector, image_list, result_path, threshold, batch_size=64):
    ...
    for start in range(0, len(image_list), batch_size):
        ...
        det_results = detector.predict(input_im_lst, input_im_info_lst)
        boxes = det_results['boxes']
        boxes_num = det_results['boxes_num'].astype('int32').flatten()

        box_start = 0
        for image_id, im_bboxes_num in zip(image_ids, boxes_num):
            box_end = box_start + int(im_bboxes_num)
